...
class Mining : public WorkmanTask {
  ...
  virtual MoveInfo getMove(Creature* c) override {
    ...
    if (!targetDeposit || !isOreDeposit(*targetDeposit)) {
    ...
    auto dir = c->getPosition().getDir(*targetDeposit);
    DestroyAction action(DestroyAction::Type::DIG);
    if (auto destroy = c->destroy(dir, action)) {
      return MoveInfo(destroy.append([this, depositPos = *targetDeposit](Creature* creature) {
        collectOre(creature, depositPos);
        incrementProgress();
        targetDeposit = none;
      }));
    }
  ...
  bool isOreDeposit(Position pos) const {
    if (auto furniture = pos.getFurniture(FurnitureLayer::MIDDLE))
      return furniture->canDestroy(DestroyAction(DestroyAction::Type::DIG));
    return false;
  }
  ...
  optional<Position> findDeposit(const Creature* c) const {
    ...
    for (auto& pos : getCollective()->getKnownTiles().getAll())
      if (isOreDeposit(pos) && pos.canNavigateToOrNeighbor(c->getPosition(), movement)) {
        auto dist = pos.dist8(c->getPosition());
        if (dist && (!bestDist || *dist < *bestDist)) {
          best = pos;
          bestDist = *dist;
  ...
  void collectOre(Creature* creature, Position pos) const {
    vector<Item*> ores;
